...
fn build_items_vec(input_type: InputType, items: Vec<Box<Expression>>) -> Option<Vec<Expression>> {
    if input_type != InputType::RANGE {
        return Some(items.iter().map(|it| it.deref().to_owned()).collect());
    }
    if items.len() != 2 {
        return None;
    }
    ...
    for (it, i) in items.iter().zip(0..) {
        let int = if let Expression::LITERAL(tok) = it.deref() {
            if let TokenType::INTEGER(s) = tok.ttype {
                s
            } else {
                return None;
            }
    ...
    if int_range[0] <= int_range[1] {
        ...
        ret = (int_range[1] + 1..int_range[0] + 1)
            .rev()
            .map(|i| Expression::LITERAL(Token::new(TokenType::INTEGER(i), 0, 0)))
            .collect::<Vec<Expression>>();
    }
...
pub fn desugar_association_declaration(
    ...
    let items = if let Some(it) = build_items_vec(input_type, items) {
        ...
        return eval.error(EVAL_INVALID_RANGE);
    };
    op_queue.push_front(Operation::from_type(ASSOC_GROWER_SETUPPER_OP(
        ValueMap::new(),
        items.len(),
        is_lazy,
    )));
