...
impl Token {
    pub fn new(ttype: TokenType, row: usize, column: usize) -> Token {
        Token {
            ttype,
...
impl<'a> Lexer<'_> {
    ...
    fn consume_num(&mut self, starting_digit: char) -> TokenType {
        ...
        if is_float {
            ...
            INTEGER(i64::from_str(str.as_str()).unwrap())
        }
    }
            ...
            match symbol {
                ...
                '\n' => {
                    self.scribe.annotate_error(Error::on_coord(
                        &Coord::new(self.current_row, self.current_column),
                        ErrorType::LEXER_BAD_STR_FMT,
                    ));
                    self.current_row += 1;
                    self.current_column = 1;
                    return STRING(str);
    ...
    pub fn produce_tokens(&mut self) -> &Vec<Token> {
        while self.can_consume() {
            ...
            if ttyp == NOTATOKEN {
                self.scribe.annotate_error(Error::on_coord(
                    &Coord::new(self.current_row, self.current_column),
                    ErrorType::LEXER_UNEXPECTED_SYMBOL(symbol),
                ));
            }
            self.tokens
                .push(Token::new(ttyp, self.current_row, self.current_column));
        }
